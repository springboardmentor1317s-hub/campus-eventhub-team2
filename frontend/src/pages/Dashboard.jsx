...
export default function Dashboard() {
  ...
  const statCard = { background: "linear-gradient(90deg, #0996e6, #29c2ee)", color: "white", padding: "1.1rem", borderRadius: "13px", textAlign: "center", fontWeight: "bold", fontSize: "1.15rem", boxShadow: "0 8px 24px #c2e7fa", letterSpacing: "1px" };
  const eventCardGrid = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(350px,1fr))", gap: "36px" };
  const eventCard = { background: "#ffffff", borderRadius: "17px", boxShadow: "0 6px 34px 0px #bacee0", padding: "2rem 1.3rem 1.2rem 1.3rem", display: "flex", flexDirection: "column", transition: "box-shadow .15s", position: "relative" };
  const eventCardRegistered = { background: "#f7fff9", borderRadius: "17px", boxShadow: "0 6px 34px 0px #e0faed", padding: "2rem 1.3rem 1.2rem 1.3rem" };
  const registerBtn = { marginTop: "18px", padding: "10px 28px", background: "linear-gradient(90deg, #0996e6, #29c2ee)", color: "white", fontWeight: 600, border: "none", borderRadius: "9px", fontSize: "1.07rem", cursor: "pointer", boxShadow: "0 2px 7px #cde8fa", transition: "background .18s", alignSelf: "start" };
  const registeredBtn = { ...registerBtn, background: "gray", cursor: "not-allowed" };
  const selectStyle = { padding: "8px 12px", borderRadius: "6px", border: "1px solid #0996e6", fontSize: "1rem", color: "#14476f", cursor: "pointer", transition: "all 0.2s ease" };
        ...
        {user.role === "college_admin" && (
          <>
            ...
            <section style={{ marginTop: "2rem" }}>
              ...
              {sortedEvents.length === 0 ? <p>No upcoming events.</p> : (
        ...
        {user.role === "student" && (
          <>
            ...
            <section>
              ...
              {displayEvents.length === 0 ? <p>No events available.</p> : (
                <div style={eventCardGrid}>
                  {displayEvents.map((event) => {
                    ...
                    return (
                      <div key={event._id} style={isRegistered ? eventCardRegistered : eventCard}>
                        ...
                        <strong style={{ fontSize: "1.3rem", color: "#14476f" }}>{event.title}</strong>
                        <span style={{ background: isRegistered ? "#e9fff0" : "#e4f1fb", color: isRegistered ? "#1b7e59" : "#2384cb", marginLeft: "10px", padding: "3px 12px", borderRadius: "18px", fontSize: "0.98rem" }}>{event.category}</span>
                        <div style={{ margin: "10px 0 14px 0", color: "#666", fontSize: "1.04rem" }}>📍 {event.location || "N/A"}</div>
                        <div style={{ marginBottom: "10px", color: "#444", fontSize: "1rem" }}>🏫 {event.college || "N/A"}</div>
                        <div style={{ color: "#888", fontSize: "0.97rem", marginBottom: "8px" }}>{new Date(event.startDate).toLocaleDateString()} - {new Date(event.endDate).toLocaleDateString()}</div>
                        <button style={isRegistered ? registeredBtn : registerBtn} onClick={() => !isRegistered && handleRegister(event._id)} disabled={isRegistered}>
